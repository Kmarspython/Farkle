...
def main():
    ...
    board1_rects = []
    # List that tells which dice in board1 should be highlighted
    ...
    while running:
        ...
        can_keep, points = keeping(selected_dice, one_combos, two_combos, three_combos,
                                   four_combos, five_combos, six_combos)
        # Determines the color of each button based on the button flags
        keep_color = p.Color("green") if can_keep is True else p.Color("gray")
        roll_color = p.Color("green") if can_roll is True else p.Color("gray")
        done_color = p.Color("green") if can_done is True else p.Color("gray")
        ...
        for e in p.event.get():
            if e.type == p.QUIT:
                ...
                if not farkled:
                    ...
                    if button_collision(keep, pos) and can_keep is True:
                        ...
                        for r in range(len(highlighted_dice)):
                            if highlighted_dice[r]:
                                board1[r] = "remove"
                        board1 = [item for item in board1 if item != "remove"]
                ...
                if button_collision(done, pos) and can_done:
                    ...
                    if farkled:
                        previous_points = 0
                        total_points = 0
                    # If the player has not farkled then they get to score the points
                    # and it is displayed as previous points
                    else:
        ...
        roll_inflate = True if button_collision(roll, pos) and can_roll is True else False
        keep_inflate = True if button_collision(keep, pos) and can_keep is True else False
        done_inflate = True if button_collision(done, pos) and can_done is True else False
        ...
        create_text(screen, f"Previous Points: {str(previous_points)}", my_font, p.Color("Red"),
                    (140, HEIGHT // 2 + 20))
        if farkled:
...
def draw_dice(board1, board2, screen):
    ...
    for r in range(len(board1)):
        ...
        x = ((2 * r + 4) * DICE_SIZE) + (DICE_SIZE * onetotwo[x1]) if r <= 2 else \
            ((2 * (r - 3) + 4) * DICE_SIZE) + (DICE_SIZE * onetotwo[len(board1) - 3])
...
def create_button(screen, font, big_font, text, text_color, button_center, button_width, button_height,
                  button_color, inflate):
